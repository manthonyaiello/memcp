...
--  transport, and owns the Store + Embedder singletons the tools run against.
--  Ada owns `main`, so elaboration stays automatic.
...
procedure Memcp with SPARK_Mode => On is
   ...
   --  The generic MCP core, specialized to memcp's 9 tools. Parse_Envelope is
   --  the one json-dependent formal -- memcp supplies it (Memcp_Envelope, built
   --  on the json crate) so spark_mcp itself stays json-free. Invoke reaches the
   --  Store/Embedder through Memcp_Resources (its seam has nowhere to pass them).
   ...
   --  The transport, specialized to the core's Dispatch. Both seams are now
   ...
   --  otherwise the response body is copied into a transport allocation and the
   --  core's allocation is freed. The one copy lives here in the composition
   --  root, off the proven hot path -- it is the price of keeping the core and
   --  its transport free of any shared pointer type (neither uses the other's).
...
begin
   Memcp_Resources.Open (DB_Path, Model_Path, Open_St);
   ...
   Ada.Text_IO.Put_Line
     ("memcp serving on http://127.0.0.1:" & Port'Image
      & " (db=" & DB_Path
      & ", embedder="
      & (if Memcp_Resources.Embedder_Loaded
         then "loaded"
         else "off [" & Model_Path & "]") & ")");
