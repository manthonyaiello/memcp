...
--  Parse_Envelope function decodes request text into. Non-generic, because
--  Spark_Mcp.Server's formal returns the type and so must be able to name it
...
package Spark_Mcp.Requests with SPARK_Mode => On is
   ...
   --    (=> Invalid_Request).
   --  @enum Unimplemented The parser cannot decode the request
   --    (=> Internal_Error). Also the default Kind below, so a parser that
   --    decides nothing reports it; a conforming parser never returns it.
   ...
   with Dynamic_Predicate =>
     ...
     --  lengths of every non-Parsed Kind trivially do.
